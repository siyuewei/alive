...
import DetailedCard from "../Component/ChartCard/DetailedCard";
import {Grid, Card, Typography, Container} from "@mui/material";
import { Statistic } from 'antd';
import CountUp from 'react-countup';
import '../App.css'
const CalorieDetailView = () => {
        ...
        <Grid container spacing={0} >
            ...
            <Grid item xs={12} md={4} style={{ width:'40%',display: "flex",flexDirection:'column', alignItems: "center" }}>
                <Grid item className={'grid-center'} >
                    <Typography variant="body1" style={{color:'black'}}>本周平均卡路里摄入量</Typography>
                </Grid>
                <Grid item className={'grid-center'}>
                    <Statistic title="" value={average_data3} precision={2} formatter={formatter} style={{marginBottom:'20px',fontSize:'20px'}} />
                </Grid>
                <Card style={{width:'85%' ,display: "flex",justifyContent:"center", alignItems: "center"}}>
                    <Grid container direction={"column"}>
                    <Grid item xs={12} sx={{ pt: '16px !important' }}>
                        <SmallChartCard chartType={ChartType.PIE} data={distribute}
                                        categries={labels} title={'饮食分布'} customize_date={'本周'} />
                    </Grid>
                        <Grid item xs={12} sx={{ pt: '16px !important' }}>
                            <Grid container>
                                <Grid item className={'grid-center'} >
...
export default CalorieDetailView;